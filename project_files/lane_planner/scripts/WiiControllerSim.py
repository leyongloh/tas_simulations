...
class TeleopKeyboard:
    ...
    def run(self):
        ...
        # 1500 is zero velocity in arduino encoding according to tas_autonomous_control C++ files.
        x = 1500
        y = 1500
        try:
            print(self._msg)
            while not rospy.is_shutdown():
                key = self._getKey()
                # Assings earlier defined speed and turn values to the control vector.
            ...
            ctrl_vec.y = 1500
            self._ctrl_pub.publish(ctrl_vec)
            # Reset input stream state after operation.
    ...
    def _getKey(self):
        ...
        # Reset input stream after key press.
        termios.tcsetattr(sys.stdin, termios.TCSADRAIN, self._settings)
        return key
...
def main():
    ...
    try:
        wii_ctrl.pub = rospy.get_param('/WiiControllerSim/autonomous')
    except rospy.ROSException:
        print("Parameter Error")

    # Run the forwarding in seperate thread to make input possible.
    thread1 = threading.Thread(target=wii_ctrl.run, args=())
    thread1.daemon = True
    thread1.start()
    ...
    while not rospy.is_shutdown():
        ...
        if cmd == '':
            if wii_ctrl.pub == False:
                ...
                teleop.run()
                # User has manually stopped teleoperation, therefore autonomous mode is switched back on.
                print('Started autonomous mode!')
                wii_ctrl.pub = True
                print(wii_msg)
        elif cmd in ['q', 'Q']:
...
if __name__ == '__main__':
    # Register a different signal handler to make clean interrupts with threading and rospy possible.
